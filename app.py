...
def upload_dec():
    ...
    if input_hash == decrypted_msg_string[0:64]:
        ...
        return render_template("final.html",ext='FAILED !! Incorrect data...')
...
def upload_file():
    ...
    bill.write("\n"+str(private[0])+","+str(hash_of_message.hexdigest())+","+str(filename))
    bill.close()
    
    
    return render_template("shortenurl.html",hash=private[0],hash1=hash_of_message.hexdigest(), user_image = filename)
